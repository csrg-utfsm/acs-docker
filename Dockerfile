...
RUN yum update && yum install -y epel-release &&\
    ...
    yum update && yum install -y make bzip2 unzip patch libX11-devel git wget gcc gcc-c++ emacs antlr expat-devel cppunit cppunit-devel swig log4cpp castor xerces-c xerces-c-devel xerces-j2 ksh subversion autoconf doxygen blas-devel byacc vim subversion openssh time bc flex gsl-devel openldap-devel procmail python-pip python-devel openssl-devel libxslt-devel libxml2-devel sqlite-devel freetype-devel libpng-devel bzip2-devel
